...
// Update the handleProductHover function
const handleProductHover = useCallback(async (product, event) => {
  ...
  // Set timer to show the similar products after 500ms
...
// Add product card mouse leave handler
const handleMouseLeave = () => {
  ...
  // Don't hide the popup here - user might be moving to the popup
};

// Update the similar products popup mouse leave handler
    ...
    <div className={`flex flex-col min-h-screen ${darkMode ? 'bg-gray-900 text-white' : 'bg-gray-50 text-black'}`}>
      ...
      <main className="flex-grow py-6 px-6">
        <div className="w-full mx-auto">
          ...
          <div className={`${darkMode ? 'bg-gray-800 border-gray-700' : 'bg-white border-gray-200'} p-6 rounded-lg shadow-sm mb-6 border-2`}>
            <form onSubmit={handleSearch} className="flex items-center gap-4">
              ...
              <select
                ...
                } focus:outline-none focus:border-[#FE90EA] focus:ring-1 focus:ring-[#FE90EA]`}
              >
          ...
          <div className="flex flex-col lg:flex-row gap-6">
            ...
            <div className="lg:w-1/3">
                ...
                setQuery={setQuery} 
                handleSearch={() => performSearch(query)} // Pass an inline function that calls performSearch with the current query
                darkMode={darkMode}
              />
            ...
            {similarProducts.length > 0 ? (
              similarProducts.map((product, index) => (
                ...
                >
                  ...
                  <div className="ml-3 flex-grow min-w-0"> {/* Added min-w-0 to enable text truncation */}
                    <div className="flex justify-between items-start w-full">
                      <h4 className={`text-sm font-medium ${darkMode ? 'text-gray-200' : 'text-gray-800'} truncate max-w-[65%]`}>{product.name}</h4>
                      {product.price_cents !== undefined && (
                        <div className={`text-sm font-medium ${darkMode ? 'text-gray-200' : 'text-gray-800'} ml-1 flex-shrink-0`}>${(product.price_cents / 100).toFixed(2)}</div>
                      )}
                    </div>
                    ...
                    <p className={`text-xs ${darkMode ? 'text-gray-400' : 'text-gray-500'} mt-1 truncate`}>{product.description}</p>
...
function ScrollingQueryExamples({ setQuery, handleSearch, darkMode }) {
